...
async function validateRuntimeAsset(
  asset: ReturnType<typeof getCuratedDeskteriorAssets>[number],
  strictWarnings: boolean,
  requireKtx2: boolean
  ...
  try {
    ...
    if (requireKtx2 && !usesKtx2 && asset.contractMetadata.textureSet.authored === "image_based") {
...
export async function buildCuratedValidationSummary(
  strictWarnings: boolean,
  requireKtx2: boolean
): Promise<ValidationSummary> {
  const curatedAssets = getCuratedDeskteriorAssets(createAssetCompilerPaths());
  const results = await Promise.all(
    curatedAssets.map((asset) => validateRuntimeAsset(asset, strictWarnings, requireKtx2))
  );
...
export function printValidationSummary(summary: ValidationSummary) {
...
export async function runValidateCuratedRuntimeCli(argv: string[]) {
  const { json, strictWarnings, requireKtx2, help, unknownArgs } = parseValidateArgs(argv);
  if (help) {
    console.log(
      [
