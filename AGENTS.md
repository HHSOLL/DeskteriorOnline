...
공통 행동 규칙은 상위 `/Users/sol/Desktop/AGENTS.md`를 반드시 따릅니다. 이 파일은
전역 지침을 대체하지 않으며, DeskteriorOnline에만 필요한 제품 방향, 문서 기준,
검증 명령만 추가합니다.

## 1) Start Protocol (작업 시작 전 필수)
1. 아래 문서를 **반드시 순서대로** 읽고 현재 기준을 확인한다.
2. 변경 범위/리스크/검증 계획을 정한 뒤 구현을 시작한다.
...
## 4) Documentation Update Rules (작업 종료 전 필수)
