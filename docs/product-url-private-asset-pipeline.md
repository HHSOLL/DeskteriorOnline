...
| Owner-scoped private generated asset listing with signed runtime sidecar URLs | `apps/api/src/repositories/assets-repo.ts`, `apps/web/src/app/api/v1/assets/route.ts` |
...
| GLB, thumbnail, and CAD/runtime sidecar storage metadata | `apps/worker/src/repositories/assets-repo.ts` |
...
| Editor catalog/runtime bridge preserves generated sidecars and runtime metadata | `apps/web/src/lib/builder/catalog.ts`, `apps/web/src/lib/runtime/legacy-scene-runtime.ts`, `apps/web/src/lib/stores/useSceneStore.ts` |
...
- `generation.pipeline` identifies the product URL private asset pipeline, including strategy-specific factory versions such as `"product_url_hybrid_asset_factory_v1"`
- `generation.strategy`
- `generation.selectedProvider`
- `generation.qualityScore`
- `generation.sidecarUploads` for uploaded CAD/runtime source, STEP placeholder/export target, runtime package, collider, support surface, attachment point, interaction anchor, material variant, and QA report sidecars when present
- `runtimeAsset.units = "mm"`
- `runtimeAsset.dimensionsMm`
- `runtimeAsset.scaleLocked`
- `runtimeAsset.categoryProfile`
- `runtimeAsset.placement`
- `runtimeAsset.sidecars` with uploaded storage paths grouped by semantic sidecar role
- `runtimeAsset.supportSurfaces`, `runtimeAsset.attachmentPoints`, `runtimeAsset.colliders`, `runtimeAsset.interactionAnchors`, and `runtimeAsset.materialVariants` when generated by CAD-first strategies
...
11. Generated CAD/runtime sidecar storage paths are persisted in private asset metadata, API responses expose owner-scoped signed sidecar URLs, and the editor runtime bridge can consume generated support surfaces, colliders, and attachment points from `runtimeAsset`.
...
Hard-surface matching must protect assembly categories from broad keyword capture. In particular, `case fan` and `120mm fan` products classify as `fan`, radiator/AIO products classify as `radiator`, and bare PC enclosure matching must not steal fan/radiator products merely because their descriptions mention a case or radiator mount.
